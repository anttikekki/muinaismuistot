...
export default function SearchPage(
  ...
  var displayResults = function(searchResults) {
    var htmlRows = searchResults.map(function(resultRow) {
      return generateResultFeatureRowHtml(resultRow);
    });

    $("#search-results-container")
      .empty()
      .html('<div class="list-group">' + htmlRows.join("") + "</div>");
  ...
  var generateResultFeatureRowHtml = function(feature) {
    ...
    return (
      '<a href="' +
      urlHashHelper.createLocationHash(coordinates, feature) +
      '" class="list-group-item">' +
      '<h4 class="list-group-item-heading">' +
      nimi +
      "</h4>" +
      '<p class="list-group-item-text"><img src="' +
      iconURL +
      '"> ' +
      tyypinNimi +
      "</p>" +
