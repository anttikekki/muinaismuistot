...
| --- | --- | --- | --- | --- |
| `validate-layer-mapping.mjs` | Kehitystarkistus, ei yöajossa | `contract/layer-mapping.json`, `config/layers.json`, UI-enum ja ladatut GeoPackaget | 12 fyysistä ja 26 loogista tasoa; tunnisteiden uniikkius; tiedostot, tasot ja vaaditut kentät; geometriaperheet; johdettujen kenttien havaitut arvojoukot; loogisten suodattimien kattavuus; UI-enumin ja build-konfiguraation vastaavuus | Konsoli |
| `validate-source-data.mjs` | Heti latauksen jälkeen | 12 GeoPackagea, mapping ja build-konfiguraatio | Tasot eivät ole tyhjiä; EPSG:3067; odotettu geometriaperhe; vain sallitut null-geometriat; kaikki geometriat valideja | `data/build/source-validation-report.json` |
...
regressiosta. Pysyvät odotusarvot kuuluvat ensisijaisesti
`contract/layer-mapping.json`- tai `processing/config/layers.json`-tiedostoon.
