...
source_etag=""
if [[ -s "$SOURCE_HEADERS" ]]; then
  source_last_modified="$(awk 'BEGIN{IGNORECASE=1} /^Last-Modified:/ {sub(/^[^:]+:[[:space:]]*/, ""); sub(/\r$/, ""); value=$0} END{print value}' "$SOURCE_HEADERS")"
  source_etag="$(awk 'BEGIN{IGNORECASE=1} /^ETag:/ {sub(/^[^:]+:[[:space:]]*/, ""); sub(/\r$/, ""); value=$0} END{print value}' "$SOURCE_HEADERS")"
fi

jq -n \
  --arg version "$version" --arg publishedAt "$published_at" \
  --arg sourceLastModified "$source_last_modified" --arg sourceEtag "$source_etag" \
  ...
  '{schemaVersion:2,version:$version,publishedAt:$publishedAt,
    sourceLastModified:($sourceLastModified | if length > 0 then . else null end),
    sourceEtag:($sourceEtag | if length > 0 then . else null end),
    state:"built",createdAt:$createdAt,sourceDigest:$sourceDigest,
...
jq '{schemaVersion,version,publishedAt,sourceLastModified,sourceEtag,createdAt,sourceDigest,compatibility,counts,endpoints,integrity:{pmtilesSha256:.artifacts.pmtiles.sha256,d1ImportSha256:.artifacts.d1Import.sha256}}' \
