...
run_step "Download source data" "$SCRIPT_DIR/download-source-data.sh"
run_step "Validate downloaded source data" node "$SCRIPT_DIR/validation/validate-source-data.mjs"
run_step "Build compact PMTiles" "$SCRIPT_DIR/build-pmtiles.sh"
...
run_step "Build D1 feature import" "$SCRIPT_DIR/build-feature-details-sql.sh"
run_step "Validate PMTiles and D1 identities" node "$SCRIPT_DIR/validation/validate-pmtiles-d1-identities.mjs"
run_step "Create checksummed build manifest" "$SCRIPT_DIR/create-build-manifest.sh"
run_step "Create timestamped release descriptor" "$SCRIPT_DIR/create-release-descriptor.sh"
