...
async function generateVocabulary([gpkg, layer, output]) {
  ...
  const vocabulary = { schemaVersion: 1, types, datings, subtypes: [...subtypeSet].sort((a, b) => a.localeCompare(b, "fi")) }
...
function mask(raw, values, kind) {
  ...
  for (const value of atomicValues(raw, kind)) {
    ...
    if (index === undefined) throw new Error(`Unknown ${kind} value: ${JSON.stringify(value)} in ${JSON.stringify(raw)}`)
    result += 2 ** index
  }
  return result
}
  ...
  for await (const line of input) {
    ...
    if (layerId === "archaeological_points") {
      ...
      properties.subtype_codes = atomicValues(source.subtypes_raw, "subtype").map((value) => {
        ...
        if (code === undefined) throw new Error(`Unknown subtype value: ${JSON.stringify(value)}`)
        return code.toString(36)
      }).join(".")
    } else if (layerId === "archaeological_areas") {
      properties.laji_key = source.laji_key
    }
