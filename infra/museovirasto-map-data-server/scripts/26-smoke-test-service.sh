...
range_status="$(curl --silent --show-error --output "$WORK_DIR/header.bin" --dump-header "$WORK_DIR/header.txt" \
  --write-out '%{http_code}' --header 'Range: bytes=0-126' "$BASE_URL/pmtiles/current.pmtiles")"
...
request_json search GET '/api/search?q=turun'
